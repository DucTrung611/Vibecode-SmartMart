...
- **Variant display is read-only, but stock-aware.** The detail page lists each variant's flattened `optionSummary` as a static chip with a `StockBadge` (from `features/inventory`) next to it; no selection/quantity UI since `cart` doesn't exist on the frontend yet.
